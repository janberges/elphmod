...
def read_hrdat(seedname, divide_ndegen=True):
    ...
    if comm.rank == 0:
        ...
        comment = next(data).strip()

        if tb:
            cell = np.array([list(map(float, cols())) for _ in range(3)])
        else:
            cell = None
        ...
        comment = cell = num_wann = nrpts = None

    comment = comm.bcast(comment)
    cell = comm.bcast(cell)
    ...
    return cells, const, cell, comment
...
def write_hrdat(seedname, R, H, ndegen=None):
    ...
    with open('%s_hr.dat' % seedname, 'w') as hr:
        hr.write(time.strftime(' written on %d%b%Y at %H:%M:%S by elphmod\n'))
...
def k2r(el, H, a=None, r=None, fft=True, rydberg=False):
    ...
    a : ndarray
        Bravais lattice vectors to overwrite :attr:`el.a`.
    r : ndarray
    ...
    el.R, el.data, l = elphmod.bravais.short_range_model(t, el.a, el.rc,
